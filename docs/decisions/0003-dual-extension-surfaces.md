...
* Sources are *configured instances*: the user passes constructor arguments (`url=`, `secret_id=`, `region=`). A plugin framework that only registers classes cannot carry those arguments without awkward factory hookspecs.
* Stateless hooks are *events*: multiple participants may run, order may matter, and no per-hook state is needed.
...
The mental model mirrors pytest: there are *fixtures* (instances you compose, with state) and there are *hooks* (events you participate in, stateless). They are different tools for different problems.
