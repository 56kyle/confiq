...
class TestEtcdSource:
    def test_missing_etcd3_raises(self, monkeypatch):
        from confiq.sources.etcd import EtcdSource

        monkeypatch.setitem(sys.modules, "etcd3", None)
        with pytest.raises(MissingDependencyError, match="confiq\\[etcd\\]"):
            EtcdSource().load()
    ...
    def test_load_nested_keys(self, monkeypatch):
        ...
        meta1 = MagicMock()
        meta1.key = b"/myapp/database/host"
        meta2 = MagicMock()
        meta2.key = b"/myapp/port"

        client_instance = MagicMock()
        client_instance.get_prefix.return_value = [
            (b"pg.local", meta1),
            (b"5432", meta2),
        ]
        etcd3_mod = MagicMock()
        etcd3_mod.client.return_value = client_instance
        monkeypatch.setitem(sys.modules, "etcd3", etcd3_mod)

        result = EtcdSource(path="/myapp").load()
    ...
    def test_protocol(self):
        ...
        assert EtcdSource.protocol == "etcd"
